...
| `onDelete` | No | Behavior when a volume is deleted: `delete`, `retain`, or `archive`. | `delete` | `delete`, `retain`, `archive` |
...
| `shareMaprootGroup` | No | Group mapped to root for NFS share. | None | `smb` |
...
### Tips
#### `dsNameTemplate` parameter supports following pv/pvc metadata conversion:
> if `dsNameTemplate` value contains following strings, it would be converted into corresponding pv/pvc name or namespace
 - `${pvc.metadata.name}`
 - `${pvc.metadata.namespace}`
 - `${pv.metadata.name}`
> if `dsNameTemplate` does not contains`${pv.metadata.name}`, the driver will append it to the template. The maximum length of the dataset name can not exceed 200 bytes minus the length of the`dsArchivePrefix` parameter. If the length exceed 200, the name will be shorten to 200 and the last chars replaced why a unique hash value
