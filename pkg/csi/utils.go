...
func truncateWithHash(s string, maxLength int) string {
	...
	hashStr := hashString(s, hashLength)

	// Replace last 15 chars with the hash
	return s[:maxLength-hashLength] + hashStr
}

func hashString(stringToHash string, maxLength int) string {
	hash := sha256.Sum256([]byte(stringToHash))

	num := new(big.Int).SetBytes(hash[:]) // Convert bytes to a big integer
	base := big.NewInt(62)
	result := ""

	for num.Sign() > 0 && len(result) < maxLength {
		mod := new(big.Int)
...
func isArchivePrefixValid(s string) bool {
	re := regexp.MustCompile(`^[A-Za-z0-9_]+[^_]$`)
